...
namespace Essence.Framework.Model
{
    ...
    /// Structure to be used as a return type. The result can be a success and contain a value or it can be a failure.
    /// <para/>
    /// To use it, just define <see cref="Result{TValue}"/> as return type of a method. Then, use methods of
    /// <see cref="Result"/> to create the result; <see cref="Result.Success{TValue}"/> or <see cref="Result.Failure"/>.
    /// These will be implicitly converted to <see cref="Result{T}"/>.<para/>
    /// To consume the result, use
